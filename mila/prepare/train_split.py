...
def split_categories(categories, equal_splits=True):
    ...
    shutil.rmtree(
        os.path.join(config.IMAGE_DIRECTORY, "validation"), ignore_errors=True
    )
    ...
    if min_images < 100:
        logger.info(
            "At least one class has less than 100 samples, will not create validation set"
        )

    if equal_splits:
        logger.info(
            "Splitting categories equally, maximum {} images".format(min_images)
        )
    ...
    for category in categories:
        ...
        logger.info(
            "{} trains and {} validation images".format(
        ...
        os.makedirs(
            os.path.join(config.IMAGE_DIRECTORY, "train", category), exist_ok=True
        )
        os.makedirs(
            os.path.join(config.IMAGE_DIRECTORY, "validation", category), exist_ok=True
        )
        ...
        for validation_image in validation_images:
            shutil.copy(
                ...
                os.path.join(
                    config.IMAGE_DIRECTORY, "validation", category, validation_image
                ),
            )
